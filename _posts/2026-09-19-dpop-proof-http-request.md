...
受信側 Server は §4.3 に従い、`alg` が登録済みの asymmetric digital signature algorithm であり、アプリケーションでサポートされ、local policy 上許容されていることを確認します。また、`jwk` に含まれる公開鍵で JWT signature を検証しなければなりません（MUST）。
